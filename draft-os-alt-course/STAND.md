...
**Chrome: «Не защищено» при «сертификат действителен»:** чаще всего nginx отдаёт **только листовой** сертификат, без **промежуточного CrocCA** в той же цепочке — в цепочке должно быть **минимум 2** блока `BEGIN CERTIFICATE` в файле **`practice.crt`**. Попросите у PKI/админов **PEM промежуточного CA (CrocCA)** или экспортируйте его из хранилища Windows («Промежуточные центры сертификации» → **CrocCA** → экспорт Base-64), сохраните как **`CrocCA.pem`**, затем локально: **`bash draft-os-alt-course/scripts/merge-practice-tls-chain.sh /путь/к/каталогу`**, получите **`practice-fullchain.crt`** и замените им **`/etc/nginx/ssl-practice/practice.crt`** на стенде, **`nginx -t`**, **`systemctl reload nginx`**. Проверка на сервере: `echo | openssl s_client -connect 127.0.0.1:443 -servername practice.croc.ru -showcerts | grep -c 'BEGIN CERTIFICATE'` — ожидается **≥ 2**.
...
### Вход через SSO: фирменная форма ADFS

Гость попадает на портал свободно (каталог публичный), авто-редиректа на IdP нет. Кнопка **«Войти через SSO»** (`/oidc/login`) ведёт сразу на **веб-форму входа ADFS** (`prompt=login`) — привычную «красивую» страницу Крока.

`prompt=login` обязателен: без него (и даже с `prompt=none`) ADFS Croc отправляет интранет-браузеры на **WIA-эндпоинт** (`…/authorize/wia`), и у пользователей без Kerberos-тикета вместо страницы входа выскакивает системный попап **Sign in** браузера — он выглядит подозрительно и не даёт пройти дальше.

Режимы `/oidc/login` (env **`OIDC_BUTTON_MODE`**, по умолчанию `forms`):

- `forms` — сразу форма ADFS;
- `silent` — сначала тихая попытка `prompt=none`, форма только при `login_required`. Включать **только после** починки WIA/Kerberos на стороне ADFS (иначе снова попап). Настоящая сквозная аутентификация «как ts/lms» требует, чтобы админы ADFS корректно отдавали WIA этим браузерам, а fs.croc.ru был в интранет-зоне.

Фоновый тихий вход (`OIDC_SILENT_LOGIN`, middleware `TrySilentSso`) выключен по той же причине. Cookie `portal_sso_probe` относится только к нему.

**Только своя учётная запись:** кнопки «Другая учётная запись» нет, `?reauth=1` игнорируется без `OIDC_ALLOW_REAUTH=true`.
