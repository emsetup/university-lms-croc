...
1. В **`.env`** задан **`TEACHER_REPORT_TOKEN`** (ссылка вида `/instruktor/kurs-progress?key=…`) — этот же **`key`** подходит для **`/adm/kurs-teoriya?key=…`** (отдельный **`COURSE_ADMIN_TOKEN`** не обязателен).
2. Опционально: **`COURSE_ADMIN_TOKEN`** — если задан и **не** совпадает с токеном преподавателя, редактор теории откроется только с ним.
...
5. Со страницы сводки преподавателя есть кнопка **«Редактировать теорию модулей»**; прямой URL: **`https://<хост>/adm/kurs-teoriya?key=<ваш_токен>`**.
