(function () {
    ...
    var hint = document.getElementById('ap-settings-learner-hint');
    ...
    function renderResults(items) {
        ...
        if (items.length === 0) {
            setHint('Никого не найдено. Уточните email или имя (сотрудников портала выбрать нельзя).', 'err');
        }
    }
            ...
            fetch(url + '?q=' + encodeURIComponent(q), {
                ...
                .then(function (data) {
                    ...
                    if (items.length === 1) {
                        ...
                        setHint('Выберите пользователя из списка.');
                    }
                })
                .catch(function () {
                    list.hidden = true;
    ...
    form.addEventListener('submit', function (e) {
        ...
        e.preventDefault();
        setHint('Сначала выберите обучающегося из списка подсказок (клик по строке).', 'err');
        input.focus();
    });
})();
