...
@section('content')
    ...
    <div class="card">
        <p class="muted">
            <a href="{{ route('admin.course.module.sections', ['courseModule' => $courseModule->id, 'key' => $adminKey]) }}">← К разделам модуля</a>
        </p>
        ...
        <form method="post" action="{{ route('admin.course.module.practice.save', ['courseModule' => $courseModule->id, 'key' => $adminKey]) }}" style="margin-top:1rem">
            @csrf
            ...
            <div style="display:grid;grid-template-columns:repeat(12,1fr);gap:0.75rem;align-items:start">
                <div style="grid-column:span 7">
                    ...
                    <div class="muted small" style="margin-top:0.35rem">
                        <a href="{{ route('admin.practice.images.index', ['key' => $adminKey]) }}">Открыть библиотеку образов</a>
                    </div>
                </div>
