@php
    ...
    $staticBars = (bool) ($staticBars ?? false);
    $users = $stats['users'] ?? [];
    ...
    $chunkOpen = $staticBars ? '<div class="ps-export-chunk" data-ps-chunk>' : '';
    $chunkClose = $staticBars ? '</div>' : '';
@endphp

{!! $chunkOpen !!}
<div class="ps-kpi-grid">
    <article class="ps-kpi ps-kpi--users">
        <p class="ps-kpi__label">Пользователей всего</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ (int) ($users['total'] ?? 0) }}
            @else
                <span class="js-ps-num" data-ps-target="{{ (int) ($users['total'] ?? 0) }}">0</span>
            @endif
        </p>
    ...
    <article class="ps-kpi ps-kpi--authors">
        <p class="ps-kpi__label">Авторов с правами</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ (int) ($authors['granted'] ?? 0) }}
            @else
                <span class="js-ps-num" data-ps-target="{{ (int) ($authors['granted'] ?? 0) }}">0</span>
            @endif
        </p>
        <p class="ps-kpi__hint">
    ...
    <article class="ps-kpi ps-kpi--content">
        <p class="ps-kpi__label">Тестов и лабораторных</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ $testsLabs }}
            @else
                <span class="js-ps-num" data-ps-target="{{ $testsLabs }}">0</span>
            @endif
        </p>
        <p class="ps-kpi__hint">
    ...
    <article class="ps-kpi ps-kpi--assigned">
        <p class="ps-kpi__label">Назначено участников</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ (int) ($participants['assigned_excl_staff'] ?? 0) }}
            @else
                <span class="js-ps-num" data-ps-target="{{ (int) ($participants['assigned_excl_staff'] ?? 0) }}">0</span>
            @endif
        </p>
        <p class="ps-kpi__hint">
    ...
    <article class="ps-kpi ps-kpi--progress">
        <p class="ps-kpi__label">С прогрессом</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ (int) ($progress['with_progress_excl_staff'] ?? 0) }}
            @else
                <span class="js-ps-num" data-ps-target="{{ (int) ($progress['with_progress_excl_staff'] ?? 0) }}">0</span>
            @endif
        </p>
        <p class="ps-kpi__hint">
    ...
    <article class="ps-kpi ps-kpi--changelog">
        <p class="ps-kpi__label">Доработок за всё время</p>
        <p class="ps-kpi__value">
            @if ($staticBars)
                {{ (int) ($changelog['total'] ?? 0) }}
            @else
                <span class="js-ps-num" data-ps-target="{{ (int) ($changelog['total'] ?? 0) }}">0</span>
            @endif
        </p>
        <p class="ps-kpi__hint">
...
{!! $chunkOpen !!}
<div class="ps-layout">
    <section class="ps-card ps-export-section">
            ...
            @foreach ($funnel as $step)
                ...
                <div class="ps-funnel__row">
                    ...
                    <div class="ps-funnel__track">
                        <div class="ps-funnel__fill @unless($staticBars) js-ps-bar @endunless"
                             @if($staticBars) style="width:{{ $w }}%" @else data-ps-width="{{ $w }}" style="width:0" @endif></div>
                    </div>
            ...
            @foreach (($content['mix'] ?? []) as $item)
                ...
                <div class="ps-bar">
                    ...
                    <div class="ps-bar__track">
                        <div class="ps-bar__fill @unless($staticBars) js-ps-bar @endunless"
                             @if($staticBars) style="width:{{ $w }}%;background:linear-gradient(90deg, {{ $item['color'] ?? '#00b956' }}, {{ $item['color'] ?? '#0d9488' }})" @else data-ps-width="{{ $w }}" style="width:0;background:linear-gradient(90deg, {{ $item['color'] ?? '#00b956' }}, {{ $item['color'] ?? '#0d9488' }})" @endif></div>
                    </div>
                </div>
            @endforeach
        </div>
                            ...
                            <td>
                                ...
                                <div class="ps-bar" style="margin-top:0.35rem;max-width:220px">
                                    <div class="ps-bar__track">
                                        ...
                                        <div class="ps-bar__fill @unless($staticBars) js-ps-bar @endunless"
                                             @if($staticBars) style="width:{{ $cw }}%" @else data-ps-width="{{ $cw }}" style="width:0" @endif></div>
                                    </div>
                                </div>
                            </td>
                            <td>
                ...
                <div class="ps-spark">
                    @foreach ($timeline as $col)
                        ...
                        <div class="ps-spark__col" title="{{ $col['label'] }}: {{ (int) $col['value'] }}">
                            <div class="ps-spark__bar @unless($staticBars) js-ps-spark @endunless"
                                 @if($staticBars) style="height:{{ $h }}px" @else data-ps-height="{{ $h }}" style="height:4px" @endif></div>
...
{!! $chunkOpen !!}
<section class="ps-card ps-export-section" style="margin-bottom:1.5rem">
    <div class="ps-card__head">
        <h2 class="ps-card__title">Список доработок функций</h2>
        <p class="ps-card__sub">всего {{ (int) ($changelog['total'] ?? 0) }} записей</p>
    </div>
    <div class="ps-changelog @if($staticBars) ps-changelog--export @endif">
                ...
                <div>
                    ...
                    @if (! empty($entry['items']))
                        <ul class="ps-changelog__items">
                            @foreach (array_slice($entry['items'], 0, $staticBars ? 4 : 2) as $item)
                                <li>{!! $entry['items_html'][$loop->index] ?? e($item) !!}</li>
                            @endforeach
                            @if (count($entry['items']) > ($staticBars ? 4 : 2))
                                <li>… ещё {{ count($entry['items']) - ($staticBars ? 4 : 2) }}</li>
                            @endif
                        </ul>
                    @endif
                </div>
