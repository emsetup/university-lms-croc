...
return [
    ...
    'modules' => [
        1 => [
            ...
            'practice' => file_get_contents(dirname(__DIR__, 2).'/config/snippets/module_01_practice_lab.md'),
        ],
        2 => [
            'letter' => 'B',
            ...
            'practice' => 'Выполните на стенде запрос версии пакетного менеджера и покажите одну команду поиска пакета. Зафиксируйте вывод в отчете.',
        ],
        3 => [
            'letter' => 'C',
            ...
            'practice' => 'Нарисуйте схему: ЦУС - сервер установок - клиент. Отметьте, какие службы из курса вы упомянули. Сдайте скрин или текст.',
        ],
        4 => [
            'letter' => 'D',
            ...
            'practice' => 'Составьте таблицу: профиль - роль сервера - сетевой стек (если указан в материалах). 3 строки.',
        ],
        5 => [
            'letter' => 'E',
            ...
            'practice' => 'Для вашей учебной ВМ укажите, какой стек сети вы наблюдаете командами из тетради. Кратко - 5-7 строк.',
        ],
        6 => [
            'letter' => 'F',
...
MD
        ...
        8 => [
            ...
            'practice' => require dirname(__DIR__, 2).'/config/snippets/module_08_practice_lab.php',
        ],
        9 => [
            'letter' => 'I',
            ...
            'practice' => require dirname(__DIR__, 2).'/config/snippets/module_09_practice_lab.php',
        ],
    ],

    'module_quizzes' => [
            1 => [
                'theory_quiz' => require dirname(__DIR__, 2).'/config/snippets/module_01_theory_quiz_questions.php',
                'module_exam' => [
                    ['q' => 'Какой тезис верен про программную платформу и готовый дистрибутив?', 'a' => ['Это всегда один артефакт', 'Платформа - про ветки разработки, дистрибутив - про поставку пользователю', 'Платформа не существует'], 'c' => 1],
                    ['q' => 'Что отражает линейка BaseAlt в учебном контексте?', 'a' => ['Только цвет логотипа', 'Продукты и документацию организации', 'Только форум'], 'c' => 1],
                    ['q' => 'Зачем фиксировать источник образа?', 'a' => ['Только для красоты отчета', 'Для воспроизводимости и поддержки', 'Чтобы скрыть версию'], 'c' => 1],
                ],
            ],
            2 => [
                'theory_quiz' => [
                    ['q' => 'Чем apt в ОС "Альт" принципиально отличается от классического Debian-apt?', 'a' => ['Ничем', 'Используется apt-rpm и пакеты в формате RPM', 'Работает только с tar'], 'c' => 1],
                    ['q' => 'Что такое "ветка" в терминах репозитория курса?', 'a' => ['Только git-ветка ядра', 'Набор пакетов с политикой обновления', 'Только цвет терминала'], 'c' => 1],
                    ['q' => 'Зачем организации политика зеркал?', 'a' => ['Чтобы запретить обновления', 'Для контроля доверенного источника и скорости', 'Только для DNS'], 'c' => 1],
                ],
                'module_exam' => [
                    ['q' => 'Почему "подключить репозиторий" - ответственное действие?', 'a' => ['Не влияет на систему', 'Меняет доступные пакеты и доверие к источнику', 'Удаляет ядро'], 'c' => 1],
                    ['q' => 'epm в курсе - это прежде всего:', 'a' => ['Редактор почты', 'Вспомогательный инструмент для работы с пакетами', 'Графический инсталлятор Windows'], 'c' => 1],
                    ['q' => 'update-kernel упоминается как:', 'a' => ['Запрещенная команда', 'Часть сценариев сопровождения ядра', 'Игра'], 'c' => 1],
                ],
            ],
            3 => [
                'theory_quiz' => [
                    ['q' => 'Что такое Alterator в контексте курса?', 'a' => ['Только текстовый редактор', 'Семейство средств администрирования', 'Только firewall'], 'c' => 1],
                    ['q' => 'Зачем связывать netinst с Alterator?', 'a' => ['Они не связаны', 'Сетевая установка - часть администрируемой экосистемы', 'Только для Wi-Fi'], 'c' => 1],
                    ['q' => 'epcnet в ЦУС - это:', 'a' => ['Случайное имя', 'Контекст сети в централизованном управлении', 'Только VPN-клиент'], 'c' => 1],
                ],
                'module_exam' => [
                    ['q' => 'Почему нельзя описывать сетевую установку как "просто dnsmasq"?', 'a' => ['Потому что dnsmasq не существует', 'Потому что это интегрированный сценарий с ролью сервисов курса', 'Потому что PXE не используется'], 'c' => 1],
                    ['q' => 'alteratorctl нужен для:', 'a' => ['Только музыки', 'Командного взаимодействия с модулями Alterator', 'Удаления диска'], 'c' => 1],
                    ['q' => 'TFTP в сценарии PXE используется для:', 'a' => ['Только логов', 'Передачи загрузочных данных по сети', 'Только DNS'], 'c' => 1],
                ],
            ],
            4 => [
                'theory_quiz' => [
                    ['q' => 'Что такое профиль установки в инсталляторе?', 'a' => ['Только тема оформления', 'Преднабор ролей и пакетов под сценарий', 'Только часовой пояс'], 'c' => 1],
                    ['q' => 'VNC-install позволяет:', 'a' => ['Только переустановить GPU', 'Удаленно вести установку', 'Только слушать музыку'], 'c' => 1],
                    ['q' => 'Выбор ядра на установке (F5) важен для:', 'a' => ['Цвета обоев', 'Совместимости оборудования и сценария', 'Только звука'], 'c' => 1],
                ],
                'module_exam' => [
                    ['q' => 'Профиль "офисный сервер" в первую очередь задает:', 'a' => ['Только обои', 'Типовой набор служб и настроек', 'Язык клавиатуры телефона'], 'c' => 1],
                    ['q' => 'systemd-networkd может быть выбран в профиле когда:', 'a' => ['Никогда', 'Это предусмотрено сценарием профиля', 'Только на macOS'], 'c' => 1],
                    ['q' => 'Минимальный профиль обычно означает:', 'a' => ['Максимум GUI', 'Минимальный набор пакетов для базовой роли', 'Только игры'], 'c' => 1],
                ],
            ],
            5 => [
                'theory_quiz' => [
                    ['q' => 'Сколько режимов управления сетью перечислено в учебной теории модуля 5 как характерных для ОС Альт?', 'a' => ['Два', 'Три', 'Четыре', 'Пять'], 'c' => 2],
                    ['q' => 'Как по файлу /etc/net/ifaces/eth0/options распознать чистый etcnet без участия NetworkManager?', 'a' => ['NM_CONTROLLED=yes, DISABLED=yes', 'NM_CONTROLLED=no, DISABLED=no', 'BOOTPROTO=dhcp всегда означает только etcnet', 'TYPE=wifi'], 'c' => 1],
                    ['q' => 'Где NetworkManager (native) хранит профили соединений?', 'a' => ['/etc/net/ifaces', '/etc/systemd/network', '/etc/NetworkManager/system-connections', '/etc/hosts'], 'c' => 2],
                ],
                'module_exam' => [
                    ['q' => 'Почему на одном интерфейсе опасно одновременно оставлять активными etcnet и NetworkManager без согласованных флагов?', 'a' => ['Ядро запрещает два IP', 'Менеджеры перетирают настройки друг у друга', 'Это разрешено по умолчанию', 'Только из эстетики'], 'c' => 1],
                    ['q' => 'Зачем в схеме etcnet для итогового /etc/resolv.conf используется resolvconf?', 'a' => ['Чтобы запретить DNS', 'Чтобы собрать DNS от активных интерфейсов в один файл', 'Только для Wi‑Fi', 'Чтобы удалить /etc/hosts'], 'c' => 1],
                    ['q' => 'Какой командой поднять интерфейс eth0 при активном systemd-networkd?', 'a' => ['ifup eth0', 'nmcli con up eth0', 'networkctl up eth0', 'iptables -A'], 'c' => 2],
                ],
            ],
            6 => [
                'theory_quiz' => require dirname(__DIR__, 2).'/config/snippets/module_06_theory_quiz_questions.php',
                'module_exam' => require dirname(__DIR__, 2).'/config/snippets/module_06_module_exam_questions.php',
            ],
            7 => [
                'theory_quiz' => require dirname(__DIR__, 2).'/config/snippets/module_07_theory_quiz_questions.php',
                'module_exam' => require dirname(__DIR__, 2).'/config/snippets/module_07_module_exam_questions.php',
            ],
            8 => [
                'theory_quiz' => require dirname(__DIR__, 2).'/config/snippets/module_08_theory_quiz_questions.php',
                'module_exam' => require dirname(__DIR__, 2).'/config/snippets/module_08_module_exam_questions.php',
            ],
            9 => [
                'theory_quiz' => require dirname(__DIR__, 2).'/config/snippets/module_09_theory_quiz_questions.php',
                'module_exam' => require dirname(__DIR__, 2).'/config/snippets/module_09_module_exam_questions.php',
            ],
    ],
